...
| A4 | Sample content is 3–5 short CC-licensed educational videos on different topics. | They are fetched ahead of time by `scripts/fetch_sample_videos.py` (yt-dlp). *Update:* the API now also accepts a link directly (D27). |
...
### D26 — Lint with Ruff, and CI on GitHub Actions
- **Lint:** Ruff with pycodestyle, pyflakes, isort, bugbear and pyupgrade rules (`pyproject.toml`). FastAPI's `Depends`/`File`/`Form` defaults are allowed by design, and `alembic` is declared third-party because the repo's `alembic/` folder would otherwise make isort treat it as local. The existing code needed only small fixes (unused import, `datetime.UTC`, explicit `zip(strict=...)`, `raise ... from exc`).
- **Formatter:** at first not enforced, because running `ruff format` would rewrite most files. *Update:* adopted later in one dedicated, behaviour-free commit (30 files, layout only; the LLM prompt strings were checked to be identical). The line length is 120 for both lint and format. CI runs `ruff format --check`, and `.pre-commit-config.yaml` runs Ruff (lint + format) plus whitespace, end-of-file, YAML/TOML, merge-conflict and large-file checks on every local commit, with the same Ruff version pinned as in CI.
- **CI:** `.github/workflows/ci.yml` runs `ruff check` and the offline `pytest` suite on Python 3.12, installing the same locked dependencies as the Docker image (CPU torch, then `requirements.lock` + `requirements-dev.lock`).
- **Ruff is pinned in the workflow, not in the lock files.** Regenerating the locks re-resolves every runtime pin (a trial run moved `langchain-core` and dropped `greenlet`), which is out of proportion for adding a lint tool. *Trade-off:* the Ruff version lives in `ci.yml` rather than `requirements-dev.lock`; move it into the locks the next time they are regenerated for another reason.

### D27 — Accept a video link (`url`) and download only the audio, in the background job
- **What:** `POST /videos` takes a third, mutually exclusive input, `url`, alongside `file` and `sample_path`. The request only validates the link and returns 202. The background job downloads it (new status `downloading`), then runs the normal pipeline. If no `title` is given, the video's own title is used.
- **Audio only:** yt-dlp fetches the best audio track (English first, because YouTube also serves dubbed tracks). It is about 10× smaller than the video (7 MB for a 7.5-minute lecture, downloaded in about 7 s), and a single stream needs no ffmpeg merge. faster-whisper decodes the DASH `.m4a` through PyAV (verified on the full 452 s file). *Alternative rejected:* downloading the video like `scripts/fetch_sample_videos.py`, which needs an ffmpeg binary to merge the streams and wastes bandwidth on frames the pipeline never uses.
- **Download in the job, not the request:** a download takes seconds to minutes, so it follows the same background path as transcription. It gets the same idempotency: `source_path` is recorded once the file is on disk, so a retry or a re-delivered job reuses it instead of downloading again.
- **Safety:**
  - Hosts are limited to an allowlist (`URL_ALLOWED_DOMAINS`, default YouTube and Vimeo, subdomains included). This also rules out the server being made to fetch internal addresses (SSRF), such as a cloud metadata endpoint. *Alternative rejected:* accepting any URL and blocking private IP ranges. yt-dlp's generic extractor follows redirects and embedded links, so blocking addresses is harder to get right than allowing hosts.
  - The length (`URL_MAX_DURATION_S`, default 1 h) is checked from the metadata **before** downloading, to protect CPU time and the Groq quota. Playlists and live streams are refused, and the file size is capped at `MAX_UPLOAD_MB`.
  - Download errors are permanent: the video is marked `failed` with yt-dlp's reason (for example, "Video unavailable").
- **Schema:** a migration adds `videos.source_url` and makes `source_path` nullable, because a link video has no file until the job runs.
- **Dependency:** `yt-dlp` moved from the dev requirements to the runtime ones. Its exact pin (`2026.8.19`, which has no dependencies) moved from `requirements-dev.lock` to `requirements.lock` by hand, instead of re-running `lock_requirements.sh`, which would also re-resolve every other runtime pin (see D26).
- **Known limitation:** YouTube changes often. yt-dlp warns that YouTube extraction without a JavaScript runtime (deno) is deprecated and that some formats may be missing. Audio-only downloads work today; if they stop working, upgrade yt-dlp or add deno to the image.
- **Tests:** API tests replace the downloader with a fake, so they never hit the network. The downloader's own checks (length, live, playlist, error, size) run against a stubbed `yt_dlp` module. The allowlist tests include `169.254.169.254` and a look-alike host (`youtube.com.evil.example`).
